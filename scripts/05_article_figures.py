...
UPDATED with OPTIMIZED results from final ensemble model
"""
...
# ==================== 1. MAIN PREDICTION SCATTER PLOTS (ENSEMBLE) ====================
def create_prediction_scatter_plots(predictions_path, output_dir):
    """Create publication-ready scatter plots of ensemble predictions vs true values"""
    ...
    # G Plot - Using Ensemble predictions
    ax = axes[0]
    ax.scatter(pred_df['True_G'], pred_df['Ensemble_G'], 
              alpha=0.6, s=20, edgecolors='none', color='#0072B2', rasterized=True)
    
    # Perfect prediction line
    min_val = min(pred_df['True_G'].min(), pred_df['Ensemble_G'].min())
    max_val = max(pred_df['True_G'].max(), pred_df['Ensemble_G'].max())
    ax.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=1.5, label='Perfect prediction')
    
    # Add R² text
    r2 = r2_score(pred_df['True_G'], pred_df['Ensemble_G'])
    ...
    ax.set_title('(a) Strain Energy Release Rate (Ensemble)')
    ax.set_aspect('equal')
    ax.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
    
    # Velocity Plot - Using Ensemble predictions
    ax = axes[1]
    ax.scatter(pred_df['True_Velocity'], pred_df['Ensemble_V'], 
              alpha=0.6, s=20, edgecolors='none', color='#D55E00', rasterized=True)
    
    # Perfect prediction line
    min_val = min(pred_df['True_Velocity'].min(), pred_df['Ensemble_V'].min())
    max_val = max(pred_df['True_Velocity'].max(), pred_df['Ensemble_V'].max())
    ax.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=1.5)
    
    # Add R² text
    r2 = r2_score(pred_df['True_Velocity'], pred_df['Ensemble_V'])
    ax.text(0.05, 0.95, f'R² = {r2:.4f}', transform=ax.transAxes, 
            fontsize=11, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='none'))
    
    ax.set_xlabel('True Crack Velocity (μm/s)')
    ax.set_ylabel('Predicted Crack Velocity (μm/s)')
    ax.set_title('(b) Crack Velocity (Ensemble)')
    ax.set_aspect('equal')
    ax.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)
...
# ==================== 2. RESIDUAL ANALYSIS PLOTS (ENSEMBLE) ====================
def create_residual_analysis(predictions_path, output_dir):
    """Create comprehensive residual analysis plots for ensemble model"""
    
    pred_df = pd.read_csv(predictions_path)
    
    # Calculate residuals using ensemble predictions
    pred_df['G_Residual'] = pred_df['True_G'] - pred_df['Ensemble_G']
    pred_df['V_Residual'] = pred_df['True_Velocity'] - pred_df['Ensemble_V']
    ...
    ax1.scatter(pred_df['Ensemble_G'], pred_df['G_Residual'], 
    ...
    ax1.set_title('(a) G Residuals (Ensemble)')
    ...
    ax4.scatter(pred_df['Ensemble_V'], pred_df['V_Residual'], 
    ...
    ax4.set_title('(d) Velocity Residuals (Ensemble)')
...
# ==================== 4. MODEL COMPARISON BAR PLOT (UPDATED WITH OPTIMIZED RESULTS) ====================
def create_model_comparison_plot(output_dir):
    """Create bar plot comparing all models - USING OPTIMIZED RESULTS"""
    
    # OPTIMIZED RESULTS from your final ensemble model
    # Linear Regression, Ridge, Random Forest from baseline_results
    # Optimized MTL and Ensemble from optimized_mtl_results
    
    models = ['Linear\nRegression', 'Ridge\nRegression', 'Random\nForest', 
              'Optimized\nMTL', 'Ensemble\n(MTL+RF)']
    
    # UPDATED WITH OPTIMIZED RESULTS
    g_r2 = [0.9945, 0.9907, 0.9970, 0.9955, 0.9982]  # G R² values
    v_r2 = [0.7552, 0.7530, 0.9878, 0.9725, 0.9844]  # Velocity R² values
    ...
    ax.set_title('Model Performance Comparison (Optimized Results)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
# ==================== 5. PUBLICATION TABLES (UPDATED) ====================
def create_results_table(output_dir):
    """Create publication-ready results table with optimized results"""
    
    # Table 1: Model Performance Comparison - UPDATED WITH OPTIMIZED RESULTS
    table1_data = {
        'Model': ['Linear Regression', 'Ridge Regression', 'Random Forest', 
                  'Optimized MTL', 'Ensemble (MTL+RF)'],
        'G_R²': [0.9945, 0.9907, 0.9970, 0.9955, 0.9982],
        'G_RMSE': [0.0045, 0.0058, 0.0033, 0.0040, 0.0028],
        'G_MAE': [0.0016, 0.0017, 0.0004, 0.0015, 0.0012],
        'Velocity_R²': [0.7552, 0.7530, 0.9878, 0.9725, 0.9844],
        'Velocity_RMSE': [37.86, 38.02, 8.25, 12.68, 9.43],
        'Velocity_MAE': [15.51, 14.80, 1.27, 3.52, 2.85]
    }
    
    df1 = pd.DataFrame(table1_data)
    df1.to_csv(os.path.join(output_dir, 'Table1_model_comparison.csv'), index=False)
    
    # Table 2: Detailed Ensemble Results
    table2_data = {
        'Metric': ['R² Score', 'RMSE', 'MAE', 'Explained Variance', 'Max Error'],
        'G (J/m²)': [0.9982, 0.0028, 0.0012, 0.9982, 0.0156],
        'Velocity (μm/s)': [0.9844, 9.43, 2.85, 0.9845, 45.21]
    }
...
Random Forest & 0.9970 & 0.0033 & 0.0004 & 0.9878 & 8.25 & 1.27 \\
Optimized MTL & 0.9955 & 0.0040 & 0.0015 & 0.9725 & 12.68 & 3.52 \\
\textbf{Ensemble (MTL+RF)} & \textbf{0.9982} & \textbf{0.0028} & \textbf{0.0012} & \textbf{0.9844} & \textbf{9.43} & \textbf{2.85} \\
\hline
\end{tabular}
\end{table}
"""
...
def print_summary_statistics(predictions_path, output_dir):
    """Print comprehensive summary statistics using ensemble predictions"""
    
    pred_df = pd.read_csv(predictions_path)
    
    print("\n" + "="*80)
    print("SUMMARY STATISTICS FOR PUBLICATION (ENSEMBLE MODEL)")
    print("="*80)
    ...
    # G statistics using ensemble predictions
    print("\n📈 G (J/m²) PREDICTION STATISTICS (Ensemble):")
    g_r2 = r2_score(pred_df['True_G'], pred_df['Ensemble_G'])
    g_rmse = np.sqrt(np.mean((pred_df['True_G'] - pred_df['Ensemble_G'])**2))
    g_mae = np.mean(np.abs(pred_df['True_G'] - pred_df['Ensemble_G']))
    g_max_error = np.max(np.abs(pred_df['True_G'] - pred_df['Ensemble_G']))
    g_mean_error = np.mean(pred_df['True_G'] - pred_df['Ensemble_G'])
    g_std_error = np.std(pred_df['True_G'] - pred_df['Ensemble_G'])
    ...
    # Velocity statistics using ensemble predictions
    print("\n📈 CRACK VELOCITY (μm/s) PREDICTION STATISTICS (Ensemble):")
    v_r2 = r2_score(pred_df['True_Velocity'], pred_df['Ensemble_V'])
    v_rmse = np.sqrt(np.mean((pred_df['True_Velocity'] - pred_df['Ensemble_V'])**2))
    v_mae = np.mean(np.abs(pred_df['True_Velocity'] - pred_df['Ensemble_V']))
    v_max_error = np.max(np.abs(pred_df['True_Velocity'] - pred_df['Ensemble_V']))
    v_mean_error = np.mean(pred_df['True_Velocity'] - pred_df['Ensemble_V'])
    v_std_error = np.std(pred_df['True_Velocity'] - pred_df['Ensemble_V'])
    ...
    with open(os.path.join(output_dir, 'publication_summary.txt'), 'w', encoding='utf-8') as f:
        f.write("PUBLICATION SUMMARY - Ensemble Model (PA-MTL + Random Forest)\n")
        f.write("="*60 + "\n\n")
        f.write(f"Dataset: {len(pred_df)} samples\n\n")
        f.write("G (J/m²) Prediction (Ensemble):\n")
        f.write(f"  R² = {g_r2:.4f}\n")
        ...
        f.write("Crack Velocity (um/s) Prediction (Ensemble):\n")
        f.write(f"  R² = {v_r2:.4f}\n")
...
if __name__ == "__main__":
    # Set paths - UPDATED to use optimized results
    RESULTS_DIR = "E:/materials2/RAA/optimized_mtl_results"  # Changed from physics_aware_mtl_results2
    OUTPUT_DIR = "E:/materials2/RAA/publication_figures_optimized"
    ...
    # Find predictions file from optimized results
    predictions_path = os.path.join(RESULTS_DIR, 'optimized_predictions.csv')
    history_path = os.path.join(RESULTS_DIR, 'training_history.csv')  # Note: may need adjustment
    
    # If training_history.csv doesn't exist, look for it in the optimized results
    if not os.path.exists(history_path):
        # The optimized script may not save training_history.csv separately
        # It's embedded in the JSON file. We'll skip Figure 3 if not found.
        history_path = None
        print("   Note: Training history file not found. Figure 3 will be skipped.")
    
    print("="*80)
    print("GENERATING PUBLICATION-READY FIGURES AND TABLES")
    print("USING OPTIMIZED ENSEMBLE RESULTS")
    print("="*80)
    ...
    if not os.path.exists(predictions_path):
        ...
        print("   Looking for available files in results directory:")
        if os.path.exists(RESULTS_DIR):
            ...
            print("   Please run 02_comprehensive_analysis.py first to generate optimized results.")
        exit(1)
    
    # Generate all figures
    print("\n📊 Creating Figure 1: Prediction Scatter Plots (Ensemble)...")
    create_prediction_scatter_plots(predictions_path, OUTPUT_DIR)
    
    print("\n📊 Creating Figure 2: Residual Analysis (Ensemble)...")
    ...
    if history_path and os.path.exists(history_path):
        ...
        print("   ⚠️  Training history file not found. Figure 3 skipped.")
        print("   (The optimized training history is available in optimized_results.json)")
